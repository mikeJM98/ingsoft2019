			...
			data: 'accion=6&dni='+$("#dni").val(),
	     		...
	     		success: function(data) {
	     			...
	     			if(datos==""){
	     				...
	     				$("#nombres").val(datos[0]["c_nombres"]);
	                  	$("#celular").val(datos[0]["c_celular"]);
	                  	$("#direccion").val(datos[0]["c_direccion"]);
	                  	$("#tipo_cliente").val(datos[0]["c_tipocliente"]);
	     			}
	     		}
		});
	}
...
function guardar(){
	$.ajax({
		...
		success: function(data) {
			if (data==1) {
				alert('Este cliente con DNI: '+$("#dni").val()+' !ya esta Registrado');
			}else{
				if ($('#nombres').val().trim() === '') {
					$('#nombres').val("").focus();
				     	alert('ERROR!!! El cliente esta sin nombre'); return false;
				}
